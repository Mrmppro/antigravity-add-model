...
exports.hasProtectedText = hasProtectedText;
exports.isVerificationStale = isVerificationStale;
exports.resolveRoute = resolveRoute;
exports.resolveFallbackRoutes = resolveFallbackRoutes;
...
function classifyRequest(features) {
    ...
    if (features.estimatedTokens > 8000 || features.turns > 12)
        return 'complex';
    if (features.estimatedTokens > 1500 || features.turns > 3)
        return 'normal';
    // A one-shot, very short prompt is not worth paying a premium model for.
    if (features.estimatedTokens <= 300 && features.turns <= 1 && !features.ambiguous)
        return 'trivial';
    return 'simple';
}
function hasProtectedText(text) {
    return PROTECTED_PATTERN.test(text);
}
/** True when the model's proof of capability is older than the policy allows. */
function isVerificationStale(route, reverifyDays, now) {
    if (reverifyDays <= 0)
        return false;
    if (!route.verified.verifiedAt)
        return true;
 ...
 */
function eligibleRoutes(routes, tier, features, taskClass, health, reverifyDays, now, reasons) {
    const matches = [];
    for (const route of routes) {
        ...
        if (taskClass === 'protected' && (route.isLocal || route.tier === 'cheap'))
            continue;
    ...
    return matches.sort(tiering_1.compareByPriority);
}
function resolveRoute(enabled, policyInput, features, health = {}, now = Date.now()) {
    const taskClass = classifyRequest(features);
    if (!enabled)
        return { action: 'bypass', taskClass, chain: [], reasonCodes: ['auto-disabled'] };
    ...
    const reasons = [taskClass === 'protected' ? 'protected-request' : 'mode-chain'];
    ...
    if (policy.preferLocal && taskClass !== 'protected') {
        const localRoutes = eligibleRoutes(policy.routes.filter((route) => route.isLocal), 'cheap', features, taskClass, health, policy.reverifyDays, now, reasons);
 ...
 */
function resolveFallbackRoutes(policyInput, features, excludeIds, health = {}, now = Date.now()) {
    const validation = (0, validator_1.validateAutoSwitchPolicy)(policyInput);
    const policy = validation.policy;
    if (!validation.valid || policy.mode === 'off')
        return [];
    ...
    for (const tier of MODE_CHAINS[policy.mode][taskClass]) {
        for (const route of eligibleRoutes(policy.routes, tier, features, taskClass, health, policy.reverifyDays, now, reasons)) {
            if (!excludeIds.has(route.id))
                ordered.push(route);
        }
    }
